...
def simple_mig_iso(params, ns, pts):
    """
    params = (nuPre,TPre,nu1,nu2,T1,T2,m12,m21)
    ns = (n1,n2)

    Simple migration to isolation model, the population size is constant
    ...
    """
    nuPre,TPre,nu1,nu2,T1,T2,m12,m21 = params
...
# nuPre,TPre,nu1,nu2,T1,T2,m12,m21
upper_bound = [10, 1, 10, 10, 1, 1, 10, 10]
lower_bound = [1e-2, 1e-2, 1e-2, 1e-2, 0, 0, 0, 0]
p0 = [3.58393534462, 0.486773940004, 0.823557413109,  0.322669721201,  0.20555616477, 0.3, 1, 1]
...
print('###DADIOUTPUT###')
# nuPre,TPre,nu1,nu2,T1,T2,m12,m21
print('likelihood\ttheta\tN.nuPre\tT.Tpre\tN.nu1\tN.nu2\tT.T1\tT.T2\tM.m12\tM.m21')
