...
for(my $i=0;$i<@pop;$i++){
	...
	print O "$smc split --fold -o $out_dir/$split_name $pop1/model.final.json $pop2/model.final.json $split_dir/$split_name/*.smc.gz; $smc plot -g $generation_time $pdf_dir/$split_name.pdf $out_dir/$split_name/model.final.json\n";
    }
}
