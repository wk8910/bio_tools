...
my $reads_count="all.rpkm";
my $outdir="correlationship";
...
print R '
x=read.table("'."$outdir/$out_prefix.rpkm".'",header=T)
y=cor(x,method="spearman")
write.table(y,quote=F,file="'."$outdir/$out_prefix.cor".'")
';
close R;
