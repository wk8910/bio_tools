...
my $maf_record="maf_analysis.pl.record"; # input file, generated by maf_analysis.pl and maf file
my $sel_chr="ss.scaffold95060"; # selected chromosome; chromosome for plot
my $sel_start="1900001";  # start position
my $sel_end="2210000";    # end position
my $svg_file="syntenyPlotPlotter.$sel_chr.${sel_start}-${sel_end}.svg"; # output svg file name
my $width=500;
my $height=100;
my $gap=5;
...
<!-- Generator: Synteny plotter V0.1.WunWang.wk8910@gmail.com. SVG Version: 6.00 Build 0)  -->
...
<text x="'.$q_label_pos.'" y="'.$h1.'" font-family="Arial" font-size="12">'."$q_chr    $lines{$q_chr}{strand}".'</text>
<text x="'.$step.'" y="'.$h2.'" font-family="Arial" font-size="8">'.$lines{$q_chr}{start}.'</text>
...
';
    foreach my $q_mid(sort {$a<=>$b} keys %{$links{$q_chr}}){
        ...
        my $mid1=$height*0.5;
        my $mid2=$height*0.5;
        print O '<path opacity="0.2" fill="#F7000D" d="M'.$ref_x1.','."0".'H'.$ref_x2.'c'."0".','.$mid1.','.$gap_end.','.$mid2.','.$gap_end.','.$height.'H'.$q_x1.'c'."0".','.-1*$mid2.','.-1*$gap_start.','.-1*$mid1.','.-1*$gap_start.','."-$height".'z"/>
';
    }
    $step+=$origin_x2+$gap;
}
